...
module.exports = async (member) => {
    ...
    msecs -= secs * 1000;
    var string = "";// Erdem Çakıroğlu Tarafından Yapılıp Sizler İçin Paylaşılmıştır ...
    ...
    if (kurulus > 604800000) {
        const welcomechannel = member.guild.channels.cache.get(config.channels.welcomechat)
        member.setNickname(config.ayarlar.autonickname);
        member.roles.add(config.roles.unreg);
        if(welcomechannel) welcomechannel.send(`
    **:tada: ${config.ayarlar.Sunucuismi}'e** hoş geldin, ${member}!

      Sunucumuzda **\`${config.ayarlar.tag}\`** Tagımızı Alarak Kaydını Yaptırabilirsin.

      <@&${config.staffs.register}> Yetkililerimiz Sizinle İlgilenecektir
      ...
      Seninle beraber **${member.guild.memberCount}** kişi olduk! Sol tarafta bulunan <#${config.channels.voice1}> & <#${config.channels.voice2}> odalarından birine girerek kayıt işlemini gerçekleştirebilirsin.`);
    }
