...
module.exports = {
    ...
    async execute(client, message, args, embeds, timeout) {
        ...
        if(config.roles.man && config.roles.man.some(rol => user.roles.cache.has(rol))) return message.channel.send({ embeds: [embeds.setDescription("Bu kişi zaten kayıtlı.")]}).then(timeout).catch(e => { })
        await user.setNickname(`${tag} ${nick.charAt(0).toUpperCase() + nick.slice(1).toLowerCase()}${age ? ` ${config.ayarlar.nametag} ${age}` : ``}`).catch(e => { });
        await user.roles.add(config.roles.man).catch(e => { });
        await user.roles.remove(config.roles.unreg).catch(e => { });
        await user.roles.remove(config.roles.woman).catch(e => { });
        ...
        if(!regData) {
            let newRegData = new reg({
                ...
                womanReg: 0,
                manReg: 0,
                userNames: [{ nick: `${tag} ${age} ${nick.charAt(0).toUpperCase() + nick.slice(1).toLowerCase()}${age ? ` ${config.ayarlar.nametag} ${age}` : ``}`, tpye: `KIZ`}]
            }).save();
        } else {
